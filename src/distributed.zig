    ...
    pub fn scatterData(self: *DataParallel, data: *trix.DataObject) ![]trix.DataObject {
        ...
        for (0..self.world_size) |device| {
            ...
            if (device_batch_size > 0) {
                ...
                @memcpy(scattered[device].values.items, data.values.items[start_idx..end_idx]);
        ...
        if (self.rank == 0) {
            for (1..self.world_size) |device| {
                for (0..self.local_models[0].layers.items.len) |layer_idx| {
                    ...
                    @memcpy(device_layer.weights.grad_value.?.items, root_layer.weights.grad_value.?.items);
                    @memcpy(device_layer.bias.grad_value.?.items, root_layer.bias.grad_value.?.items);
                }
            }
        }
    }
        ...
        if (self.rank == 0) {
            for (1..self.world_size) |device| {
                for (0..self.local_models[0].layers.items.len) |layer_idx| {
                    ...
                    @memcpy(device_layer.weights.values.items, root_layer.weights.values.items);
                    @memcpy(device_layer.bias.values.items, root_layer.bias.values.items);
                }
            }
        }
    }
    ...
    pub fn forwardPipeline(self: *ModelParallel, input: *trix.DataObject) !trix.DataObject {
        ...
        for (0..self.num_stages) |stage| {
            ...
            for (stage_ref.layers) |*layer| {
                const output = try layer.forward(self.allocator, current_input);
    ...
    pub fn allReduce(tensors: []trix.DataObject, op: AllReduceOp, allocator: std.mem.Allocator) !void {
        ...
        switch (op) {
            .sum => {
                ...
                for (1..tensors.len) |i| {
                    @memcpy(tensors[i].values.items, tensors[0].values.items);
                }
            },
            .mean => {
